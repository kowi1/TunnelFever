 ...
 */
class TextureCubemap :public TeapotTexture {
protected:
...
class Texture2d :public TeapotTexture{
protected:
    GLuint texId_ = GL_INVALID_VALUE;
    bool activated_ = false;
public:
 ...
 */
TeapotTexture* TeapotTexture::Create( GLuint type, std::vector<std::string>& texFiles,
                       AAssetManager* assetManager) {
    if (type == GL_TEXTURE_2D) {
        return dynamic_cast<TeapotTexture*>(new Texture2d(texFiles[0], assetManager));
...
void TeapotTexture::Delete(TeapotTexture* obj) {
    if(obj == nullptr) {
